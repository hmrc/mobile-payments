...
class LivePaymentControllerISpec extends BaseISpec with MobilePaymentsTestData {
  ...
  "GET /payments/latest-payments/:utr" should {
    "return 200 with the latest payments for the user, most recent first" in {
      grantAccess()
      stubForShutteringDisabled
  ...
  "POST /payments/get-latest-payments" should {
    ...
    "return 401 when no active UTR is found on account for self assessment payments" in {
      grantAccess(activeUtr = false)
