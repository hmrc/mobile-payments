...
object ValueClassBinder {
  ...
  def bindableA[A: ClassTag: Reads](fromAtoString: A => String): QueryStringBindable[A] = new QueryStringBindable.Parsing[A](
    parse = JsString(_).as[A],
    fromAtoString,
    {
      case (key: String, e: JsResultException) =>
        s"Cannot parse param $key as ${summon[ClassTag[A]].runtimeClass.getSimpleName}. ${e.errors.headOption.flatMap(_._2.headOption.map(_.message)).getOrElse("")}"
      case (key: String, e) => s"Cannot parse param $key as ${summon[ClassTag[A]].runtimeClass.getSimpleName}. ${e.toString}"
    }
  )
